...
It is written to merge multiple collapsed biom tables under the same taxanomy format.\\n\\
It also makes sense to merge biom tables by consistent OTU IDs e.g. closed-refernce otu picking. Taxonomy column will be merged accordingly.'
epil <- 'Usage:\\n\\
merge_bioms.R -i *txt -o merge.txt\\n\\
merge_bioms.R -i *.biom -o merge.biom --biom --na2zero --all'

parser <- ArgumentParser(prog='merge_bioms.R', formatter_class= 'argparse.RawTextHelpFormatter',
                         description= dsc, epilog= epil)
parser$add_argument('-i', metavar='input', type = 'character', nargs='+', help='import biom tables.')
parser$add_argument('-o', metavar='ouput', type = 'character', nargs=1, help='Export a merged biom table.')
parser$add_argument('--na2zero', action='store_true', help='Transform NAs to zeros.')
parser$add_argument('--biom', action='store_true', help='Use biom files.')
parser$add_argument('--all', action='store_true', help='Keep all taxa, NAs will filled for unmatched taxa.')
#parser$print_help()
...
if (biom_format == TRUE){
  biom_list <- suppressWarnings(lapply(file_list, read_biom)) #warning for some biom files
...
if (ntax > 1){
  cat("OTU ID and Taxonomy are not matched consistently. Consistent OTU IDs are required to merge OTU tables")
  q(status = 1)
}
